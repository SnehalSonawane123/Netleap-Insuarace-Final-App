...
DATASET_STATS = {
    'avg_age': 39.207025,
    'avg_bmi': 30.663397,
    'avg_children': 1.094918,
    'avg_charges_usd': 13270.422265,
    'smoker_percentage': 20.478,
    'total_records': 1338,
    'smokers': 274,
    'non_smokers': 1064,
    'age_min': 18,
...
with st.sidebar:
    ...
    page = st.radio(
        "📍 Navigate to:",
        ["🏠 Home - Prediction", "📊 Detailed Analytics", "ℹ️ About"],
    ...
    if st.session_state.prediction_data is not None:
        ...
        st.metric("Risk Level", 
                 "High" if data['smoker'] == "Yes" else "Medium" if data['bmi'] > 30 else "Low")
...
if page == "🏠 Home - Prediction":
    ...
    factors = {
        'Age Factor': age * 250,
        'BMI Factor': (bmi - 25) * 100 if bmi > 25 else 0,
        'Smoking Factor': 20000 if smoker == "Yes" else 0,
        'Children Factor': children * 500,
        'Base Cost': 5000
    }
    factors_inr = {k: v * USD_TO_INR for k, v in factors.items()}
    fig_factors = px.bar(
        x=list(factors_inr.keys()), 
        y=list(factors_inr.values()),
        labels={'x': 'Cost Factor', 'y': 'Contribution (₹)'},
        title='Cost Breakdown by Contributing Factors',
        color=list(factors_inr.values()),
        color_continuous_scale='RdYlGn_r',
        text=[f"₹{v:,.0f}" for v in factors_inr.values()]
    )
    ...
    with tab1:
        ...
        with col_a1:
            ...
            fig_age.add_trace(go.Bar(
                x=['Your Age', 'Dataset Average'],
                y=[age, DATASET_STATS['avg_age']],
                marker_color=['#FF6B6B', '#4ECDC4'],
                text=[f"{age} yrs", f"{DATASET_STATS['avg_age']:.1f} yrs"],
                textposition='auto',
                textfont=dict(size=14, color='white'),
                width=0.6
            ))
            fig_age.add_hline(y=DATASET_STATS['avg_age'], line_dash="dash", line_color="gray", 
    ...
    with tab2:
        ...
        with col_b1:
            ...
            fig_bmi.add_trace(go.Bar(
                x=['Your BMI', 'Dataset Average'],
                y=[bmi, DATASET_STATS['avg_bmi']],
                marker_color=['#FF6B6B', '#4ECDC4'],
                text=[f"{bmi:.1f}", f"{DATASET_STATS['avg_bmi']:.1f}"],
                textposition='auto',
                textfont=dict(size=14, color='white'),
                width=0.6
            ))
            fig_bmi.add_hline(y=25, line_dash="dot", line_color="green", 
                             annotation_text="Healthy BMI Threshold", annotation_position="right")
            fig_bmi.add_hline(y=30, line_dash="dot", line_color="orange", 
                             annotation_text="Overweight Threshold", annotation_position="right")
            fig_bmi.add_hline(y=DATASET_STATS['avg_bmi'], line_dash="dash", line_color="gray", 
                             annotation_text="Average", annotation_position="right")
    ...
    with tab3:
        ...
        with col_c1:
            ...
            fig_children.add_trace(go.Bar(
                x=['Your Children', 'Dataset Average'],
                y=[children, DATASET_STATS['avg_children']],
                marker_color=['#FF6B6B', '#4ECDC4'],
                text=[f"{children}", f"{DATASET_STATS['avg_children']:.2f}"],
                textposition='auto',
                textfont=dict(size=14, color='white'),
                width=0.6
            ))
            fig_children.update_layout(
                title='Number of Children Comparison',
    ...
    with tab4:
        ...
        with col_d1:
            ...
            fig_cost.add_trace(go.Bar(
                x=['Your Predicted Cost', 'Dataset Average'],
                y=[prediction_inr, avg_cost_inr],
                marker_color=['#FF6B6B', '#4ECDC4'],
                text=[f"₹{prediction_inr:,.0f}", f"₹{avg_cost_inr:,.0f}"],
                textposition='auto',
                textfont=dict(size=14, color='white'),
                width=0.6
            ))
            fig_cost.update_layout(
                title='Annual Insurance Cost Comparison',
                yaxis_title='Cost (₹)',
                showlegend=False,
                height=400,
                plot_bgcolor='rgba(0,0,0,0)'
    ...
    with col_smoke1:
        ...
        fig_smoker.add_trace(go.Pie(
            labels=['Smokers', 'Non-Smokers'],
            values=[DATASET_STATS['smokers'], DATASET_STATS['non_smokers']],
        ...
        fig_smoker.update_layout(
            title=f"Dataset Smoker Distribution (Your Status: {smoker})",
            height=400,
            showlegend=True
        )
...
<head>
    ...
    <style>
        ...
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        .comparison-row .value-label {{ 
            font-size: 11px;
            color: #888;
            text-transform: uppercase;
        }}
        .comparison-row .value-number {{ 
            font-size: 18px;
            font-weight: bold;
            color: #667eea;
        }}
...
<body>
    <div class="container">
        <div class="header">
            ...
            <p>Powered by Machine Learning Predictions</p>
        </div>
        <div class="content">
            <div class="section">
                ...
                <div class="metric-grid">
                    ...
                    <div class="metric-card">
                        ...
                        <div class="metric-value">{children}</div>
                    </div>
                    <div class="metric-card">
                        <div class="metric-label">Smoker</div>
                        <div class="metric-value">{smoker}</div>
                    </div>
                    <div class="metric-card">
                ...
                <div style="text-align: center; margin-top: 20px;">
                    <span class="badge {'high' if smoker == 'Yes' else 'medium' if bmi > 30 else 'low'}">
                        Risk Level: {'High' if smoker == 'Yes' else 'Medium' if bmi > 30 else 'Low'}
                    </span>
                </div>
            </div>
            <div class="cost-highlight">
                <div class="label">Predicted Annual Insurance Cost</div>
                <div class="amount">Rs {prediction_inr:,.2f}</div>
                <div class="label">(${prediction_usd:,.2f} USD)</div>
                <div style="margin-top: 15px; font-size: 16px;">
                    {cost_diff_pct:+.1f}% {'above' if cost_diff_pct > 0 else 'below'} dataset average
                </div>
            </div>
            <div class="section">
                <h2 class="section-title">&#128202; Comparison with Dataset</h2>
                <div class="comparison-row">
                    <div class="label">Age</div>
                    <div class="values">
                        <div class="value">
                            <div class="value-label">You</div>
                            <div class="value-number">{age}</div>
                        </div>
                        <div class="value">
                            <div class="value-label">Average</div>
                            <div class="value-number">{DATASET_STATS['avg_age']:.1f}</div>
                        </div>
                        <div class="value">
                            <div class="value-label">Difference</div>
                            <div class="value-number" style="color: {'#28a745' if age < DATASET_STATS['avg_age'] else '#dc3545'}">
                                {age - DATASET_STATS['avg_age']:+.1f}
                            </div>
                        </div>
                    </div>
                </div>
                <div class="comparison-row">
                    ...
                    <div class="values">
                        <div class="value">
                            <div class="value-label">You</div>
                            <div class="value-number">{bmi:.1f}</div>
                        </div>
                        <div class="value">
                            <div class="value-label">Average</div>
                            <div class="value-number">{DATASET_STATS['avg_bmi']:.1f}</div>
                        </div>
                        <div class="value">
                            <div class="value-label">Difference</div>
                            <div class="value-number" style="color: {'#28a745' if bmi < DATASET_STATS['avg_bmi'] else '#dc3545'}">
                                {bmi - DATASET_STATS['avg_bmi']:+.1f}
                            </div>
                        </div>
                    </div>
                </div>
                <div class="comparison-row">
                    ...
                    <div class="values">
                        <div class="value">
                            <div class="value-label">You</div>
                            <div class="value-number">{children}</div>
                        </div>
                        <div class="value">
                            <div class="value-label">Average</div>
                            <div class="value-number">{DATASET_STATS['avg_children']:.1f}</div>
                        </div>
                        <div class="value">
                            <div class="value-label">Difference</div>
                            <div class="value-number" style="color: {'#28a745' if children < DATASET_STATS['avg_children'] else '#dc3545'}">
                                {children - DATASET_STATS['avg_children']:+.1f}
                            </div>
                        </div>
                    </div>
                </div>
                <div class="comparison-row">
                    ...
                    <div class="values">
                        <div class="value">
                            <div class="value-label">You</div>
                            <div class="value-number">Rs {prediction_inr:,.0f}</div>
                        </div>
                        <div class="value">
                            <div class="value-label">Average</div>
                            <div class="value-number">Rs {avg_cost_inr:,.0f}</div>
                        </div>
                        <div class="value">
                            <div class="value-label">Difference</div>
                            <div class="value-number" style="color: {'#28a745' if cost_diff < 0 else '#dc3545'}">
                                {cost_diff_pct:+.1f}%
                            </div>
                        </div>
                    </div>
            ...
            <div class="section">
                ...
                <table>
                    <thead>
                        <tr>
                            <th>Cost Factor</th>
                            <th style="text-align: right;">Contribution (Rs)</th>
                            <th style="text-align: right;">Percentage</th>
                        </tr>
                    </thead>
                    <tbody>
                        {''.join([f'<tr><td><strong>{k}</strong></td><td style="text-align: right;">Rs {v:,.2f}</td><td style="text-align: right;">{(v/sum(factors_inr.values())*100):.1f}%</td></tr>' for k, v in factors_inr.items()])}
                        <tr style="background: #667eea; color: white; font-weight: bold;">
                            <td>TOTAL</td>
                            <td style="text-align: right;">Rs {sum(factors_inr.values()):,.2f}</td>
            ...
            <div class="section">
                <h2 class="section-title">&#128161; Personalized Health Insights</h2>
                {''.join([f'<div class="insight {'danger' if '🔴' in insight else 'warning' if '🟡' in insight else 'success'}"><span class="icon">{'🔴' if '🔴' in insight else '🟡' if '🟡' in insight else '🟢'}</span>{insight.replace("🔴", "").replace("🟡", "").replace("🟢", "").strip()}</div>' for insight in insights])}
            </div>
            <div class="section">
                <h2 class="section-title">&#128200; Dataset Statistics</h2>
                <div class="stats-grid">
                    <div class="stat-item"><strong>Total Records:</strong> {DATASET_STATS['total_records']}</div>
                    <div class="stat-item"><strong>Average Age:</strong> {DATASET_STATS['avg_age']:.2f} years</div>
                    <div class="stat-item"><strong>Average BMI:</strong> {DATASET_STATS['avg_bmi']:.2f}</div>
                    <div class="stat-item"><strong>Average Children:</strong> {DATASET_STATS['avg_children']:.2f}</div>
                    <div class="stat-item"><strong>Smokers:</strong> {DATASET_STATS['smokers']} ({DATASET_STATS['smoker_percentage']:.2f}%)</div>
                    <div class="stat-item"><strong>Non-Smokers:</strong> {DATASET_STATS['non_smokers']} ({100-DATASET_STATS['smoker_percentage']:.2f}%)</div>
                    <div class="stat-item"><strong>Age Range:</strong> {DATASET_STATS['age_min']}-{DATASET_STATS['age_max']} years</div>
                    <div class="stat-item"><strong>BMI Range:</strong> {DATASET_STATS['bmi_min']:.2f}-{DATASET_STATS['bmi_max']:.2f}</div>
                    <div class="stat-item"><strong>Min Cost:</strong> Rs {DATASET_STATS['charges_min'] * USD_TO_INR:,.2f}</div>
                    <div class="stat-item"><strong>Max Cost:</strong> Rs {DATASET_STATS['charges_max'] * USD_TO_INR:,.2f}</div>
                    <div class="stat-item"><strong>Average Cost:</strong> Rs {DATASET_STATS['avg_charges_usd'] * USD_TO_INR:,.2f}</div>
                    <div class="stat-item"><strong>Currency Rate:</strong> 1 USD = Rs {USD_TO_INR}</div>
                </div>
            </div>
        </div>
        <div class="footer">
            <p><strong>Model Information:</strong> Random Forest Regressor trained on US Health Insurance Dataset</p>
            <p><strong>Dataset Source:</strong> Kaggle - US Health Insurance Dataset</p>
            <p><strong>Disclaimer:</strong> This report provides estimates based on historical data and should not be considered as actual insurance quotes.</p>
            <p>Always consult with licensed insurance professionals for accurate pricing and coverage information.</p>
            <p style="margin-top: 15px; color: #999;">Copyright 2024 Health Insurance Predictor | Report ID: {datetime.now().strftime('%Y%m%d%H%M%S')}</p>
        </div>
    </div>
</body>
</html>
...
<head>
    <meta charset="UTF-8">
    <title>Insurance Cost Summary</title>
    <style>
        body {{ font-family: Arial, sans-serif; margin: 40px; line-height: 1.6; }}
        .header {{ background: #667eea; color: white; padding: 30px; text-align: center; margin: -40px -40px 30px -40px; }}
        .cost {{ font-size: 48px; font-weight: bold; color: #667eea; text-align: center; margin: 30px 0; }}
        .section {{ margin: 20px 0; padding: 20px; background: #f8f9fa; border-left: 4px solid #667eea; }}
...
<body>
    ...
    <div class="cost">Rs {prediction_inr:,.2f}</div>
    <p style="text-align: center; color: #666;">Annual Predicted Cost</p>
    <div class="section">
        <h3>Your Profile</h3>
        <p><strong>Age:</strong> {age} years | <strong>BMI:</strong> {bmi:.1f} | <strong>Smoker:</strong> {smoker} | <strong>Children:</strong> {children}</p>
    </div>
    <div class="section">
        <h3>Comparison</h3>
        <table>
            <tr><th>Metric</th><th>You</th><th>Average</th><th>Difference</th></tr>
            <tr><td>Cost</td><td>Rs {prediction_inr:,.0f}</td><td>Rs {avg_cost_inr:,.0f}</td><td>{cost_diff_pct:+.1f}%</td></tr>
            <tr><td>Age</td><td>{age}</td><td>{DATASET_STATS['avg_age']:.1f}</td><td>{age - DATASET_STATS['avg_age']:+.1f}</td></tr>
            <tr><td>BMI</td><td>{bmi:.1f}</td><td>{DATASET_STATS['avg_bmi']:.1f}</td><td>{bmi - DATASET_STATS['avg_bmi']:+.1f}</td></tr>
        </table>
    </div>
</body>
</html>
"""
    ...
    with col_download1:
        st.download_button(
            ...
            data=report_html,
            file_name=f"insurance_report_{datetime.now().strftime('%Y%m%d_%H%M%S')}.html",
            mime="text/html",
